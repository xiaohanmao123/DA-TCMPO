...
train_dataset = load_dataset('train_demo')
test_dataset = load_dataset('test_demo')
val_dataset = load_dataset('val_demo')
...
def train_vae(model, train_loader, num_epochs=50, lr=0.001, device='cuda'):
    ...
    for epoch in range(num_epochs):
        ...
        for function, herb_features, labels, lengths, herb_id_batch in train_loader:
            ...
            z_diffused = model.diffusion(z, herb_id_batch)  
            recon_logits_diffused = model.decode(z_diffused)  

            real_loss = model.loss_function(recon_logits, labels, mean, logvar)

            diffused_loss = model.loss_function(recon_logits_diffused, labels, mean, logvar)

            loss = real_loss + diffused_loss
        ...
        with torch.no_grad():
            for function, herb_features, labels, lengths, herb_id_batch in val_loader:
                ...
                recon_logits, mean, logvar = model(function, herb_features, herb_id_batch)
                loss = model.loss_function(recon_logits, labels, mean, logvar)
                val_loss += loss.item()
        ...
        if precision > best_precision:
            torch.save(model.state_dict(), './mo.model')
            best_epoch = epoch + 1

...
def create_pts(df, name, model, device):
    ...
    for index, row in df.iterrows():
        ...
        #print('herbs_feature', herbs_feature.shape, 'function', function.shape) #herbs_feature torch.Size([768]) function torch.Size([768])
        #print('herbs_feature', herbs_feature, 'function', function, 'label', label, 'herb_ids', herb_ids)
...
train_df.to_csv('./train_demo.csv', index=False, encoding='utf-8-sig')
val_df.to_csv('./val_demo.csv', index=False, encoding='utf-8-sig')
test_df.to_csv('./test_demo.csv', index=False, encoding='utf-8-sig')
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = SentenceTransformer("./m3e-base", device=device)
herbname_id_dict = torch.load('./herbname_id_dict.pt')
create_pts(test_df, 'test_demo', model=model, device=device)
create_pts(train_df, 'train_demo', model=model, device=device)
create_pts(val_df, 'val_demo', model=model, device=device)
